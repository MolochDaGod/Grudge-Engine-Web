...
(async () => {
  ...
  // Serve the root public/ directory for ALL asset packs (kenney, quaternius, men-chars, etc.)
  // This is needed in both dev and production since large asset packs live at the project root.
  app.use(express.static(path.resolve(process.cwd(), "public"), {
    maxAge: process.env.NODE_ENV === "production" ? "1d" : 0,
    setHeaders: (res, filePath) => {
      // Allow Babylon.js to load GLB/GLTF from any origin in the same app
      if (/\.(glb|gltf|bin|ktx2|basis)$/i.test(filePath)) {
